# Ben Moffat

Hey everyone, I’m Ben.

I’m a bit of a generalist. Most of my day-job experience has been in product partnerships and commercial roles, but I love building my own products in my spare time.

Most of my background is in fintech, but what I really enjoy is solving customer problems. I like understanding what people are trying to do, where they get stuck, and then working out the simplest way to help them.

I taught myself to code during COVID-19 in 2020, and since then I’ve shipped multiple production iOS, Android and web apps with real users. Building my own products has given me the confidence that, if I set my mind to a problem and really care about it, I can solve it.

At the moment, I’m trying to learn as much about AI as possible and how it can be best implemented in engineering, product and commercial processes.

## Building now

[Conviction](https://useconviction.com) is an AI research workspace for investment analysts. It turns filings, notes and earnings calls into structured, searchable research.

`Next.js` `Supabase` `Mastra` `Langfuse`

## Shipped

- [The Next Tee](https://thenexttee.golf) - Golf course directory and leaderboard. `Next.js` `Supabase`
- [Huddle](https://apps.apple.com/gb/app/huddle-sports-club-manager/id1630535924) - Sports team management app for fixtures, availability, communication and payments. 500+ monthly active users. `Flutter` `Firebase`
- [Kalo Fitness](https://apps.apple.com/gb/app/kalo-fitness-workout-together/id1575148523) - Social workout tracking with friends. `Flutter` `Firebase`
- [Send to Santa](https://apps.apple.com/gb/app/send-to-santa-and-his-elves/id1541380178) - iOS app built during COVID-19. Top 200 kids apps, 4.5 star rating. `Swift`

## Open source

- [code-first-design](https://github.com/bmoff/code-first-design) - Notes and patterns for component-driven design and development in Next.js
- [cursor-rules-collection](https://github.com/bmoff/cursor-rules-collection) - Cursor rules for AI-assisted development workflows

## What I'm interested in

- Product work close to users
- Fintech
- AI strategy and implementation
- Customer onboarding and adoption
- Building small products from first version to live users
